...
@click.option("--yubikey/--no-yubikey", default=False, help="Try with/without YubiKey")
@click.option(
    "--yubikey-slots",
    default="1,2",
...
@click.option(
    "--try-no-password/--skip-no-password",
    default=True,
    help="Try combinations without passphrase (default: enabled)",
)
@click.option(
    "--try-no-keyfile/--skip-no-keyfile",
    default=True,
...
@click.option(
    "--quiet", "-q", is_flag=True, help="Suppress output except for success/failure"
)
def main(
    database: Path,
    ...
    if not has_passphrases and not has_keyfiles and not has_yubikey:
        click.echo(
            "Error: Must provide at least one credential type (passphrases, keyfiles, or yubikey)",
            err=True,
        )
        sys.exit(1)
